...
geometry_msgs::Vector3Stamped pitch_debug_data;
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher pitch_rt = n.advertise<geometry_msgs::Vector3Stamped>("pitch_rt",1);
  ...
  nh.param<float>("bode_omega", bode_omega, 3.0f);
  ...
  struct timeval test_start, test_now;
  double throttle_intest = 0.0;
  ...
  while (ros::ok())
  {
    ...
    case AUTO_FLYING:{
      ...
      yaw_debug_data.vector.y = pitch_global;
      ...
      joystick_output = joystick_input;
      joystick_output.buttons[JOY_CHANNEL_FLIGHT_MODE] = 1; // disable flip
      ...
      // Switch to test mode when button changes (cannot test in 1st autoflying loop)
      if(Enable_Test&& joystick_input.buttons[JOY_CHANNEL_FLIGHT_MODE] != pre_test_button){
        flight_mode = AUTO_BODE_TEST;
        gettimeofday(&test_start,NULL);
        // Record the throttle value
        throttle_intest = joystick_output.axes[JOY_CHANNEL_THROTTLE];
      }
	    pre_test_button = joystick_input.buttons[JOY_CHANNEL_FLIGHT_MODE];
    ...
    case AUTO_BODE_TEST:{
      ROS_INFO_STREAM_THROTTLE(1, "TESTING...");
      gettimeofday(&test_now,NULL);
      double testtime = test_now.tv_sec - test_start.tv_sec + 1e-6 * (test_now.tv_usec - test_start.tv_usec);

      // Attitude Sin command
      joystick_output = joystick_input;
      joystick_output.buttons[JOY_CHANNEL_FLIGHT_MODE] = 1; // disable flip
      joystick_output.axes[JOY_CHANNEL_PITCH]    = bode_amp*sin(bode_omega*testtime);
      joystick_output.axes[JOY_CHANNEL_ROLL]     = 0;
      joystick_output.axes[JOY_CHANNEL_THROTTLE] = saturate(throttle_intest*1.0, 0, 1);
      joystick_output.axes[JOY_CHANNEL_YAW]      = 0;

      // Switch back to flying mode when position offset too large
      if(VRPN_UPDATE){
        xe = saturate(transform.getOrigin().x(),  -2, 2);
        ye = saturate(-transform.getOrigin().y(), -2, 2);
        ze = saturate(transform.getOrigin().z(),  -1.2, 1.2);
        controller_pose_z.update(ze, 0, dynamic_DT);
        //controller_pose_yaw.update(saturate(-yaw, -M_PI, M_PI), 0, dynamic_DT);
        VRPN_UPDATE = false;
      }
      ...
      // Switch back to flying mode when time is off or button changes
      if(testtime > TESTTIME || joystick_input.buttons[JOY_CHANNEL_FLIGHT_MODE] != pre_test_button || fabs(xe) > xlim || fabs(ye) > ylim || fabs(ze) > zlim ){
        flight_mode = AUTO_FLYING;
    ...
    // Publish pitch data
    pitch_debug_data.header.stamp = ros::Time::now();
    pitch_debug_data.header.seq = count;
    pitch_debug_data.header.frame_id = "pitch_debug_data";
    pitch_debug_data.vector.x = 0;
    pitch_debug_data.vector.y = pitch_global;
    pitch_debug_data.vector.z = 0;
    pitch_rt.publish(pitch_debug_data);
