...
void goal_callback(const geometry_msgs::PoseStamped& message){
  ...
  q_goal.setEulerZYX(0, 0, -yaw_goal);
...
int main(int argc, char **argv)
{
  ...
  TRANSFORM_READY = false;
    ...
    // Count flip
    if (JOY_READY){
      if (!FIRSTFLIP_SKIPPED)
      {
        pre_flip_button = joystick_input.buttons[JOY_CHANNEL_FLIGHT_MODE];
        FIRSTFLIP_SKIPPED = true;
      }
      else if(pre_flip_button != joystick_input.buttons[JOY_CHANNEL_FLIGHT_MODE])
      {
        flip_count ++;
        pre_flip_button = joystick_input.buttons[JOY_CHANNEL_FLIGHT_MODE];
      }
  ...
  idletime = totaltime;
    ...
    case AUTO_FLYING:{
      ...
      throttle_sp = saturate(controller_pose_z.output, -0.05, 0.2) + TAKEOFF_THROTTLE;
  ...
  joystick_output.axes[JOY_CHANNEL_ARM] = 0;
    ...
    ROS_INFO_STREAM("FLip Count:" << flip_count);
    // if(JOY_READY)
    //    ROS_INFO_STREAM(" Flip Button: " << joystick_output.buttons[JOY_CHANNEL_FLIGHT_MODE]);
