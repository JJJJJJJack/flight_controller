...
#include<math.h>
#define delta 0.05f
#define JOY_NOB_THRESHOLD 0.1f
#define conangle 0.966f
#define wx1 -1.5f
#define wx2 1.5f
#define wy1 -1.5f
#define wy2 1.5f
...
/*class Wall{
	public:
         void setwall(double xa,double xb,double ya,double yb,double z){
  x1=xa;
  x2=xb;
  y1=ya;
  y2=yb;
...
class touchPoint{
	public:
		double x,y,z,throttle;
}touch_point;

double detect(double x,double y,double z){
	double a,b,c,d,e,f;
	a=x-wx1;
	b=wx2-x;
	c=y-wy1;
...
double detect_level(double x,double y){
	double a,b,c,d;
	a=x-wx1;
	b=wx2-x;
	c=y-wy1;
...
double detect_point(double x1,double y1,double z1,double x2,double y2,double z2){
	double r;
...
double angle(double x1,double y1,double x2,double y2){
	double ancos;
	ancos = (x1*x2 + y1*y2) / sqrt((x1*x1 + y1*y1)*(x2*x2 + y2*y2));
	return ancos;
}
...
int main(int argc, char **argv)
{
  ...
  float idletime = 0, land_height = 0;;
   ...
   //wall.setwall(wx1,wx2,wy1,wy2,wz);  
  /*wall.x1=wx1;
  wall.x2=wx2;
  wall.y1=wy1;
  wall.y2=wy2;
  ...
  while (ros::ok())
  {
    ...
    switch(flight_mode){
    case PASS_THROUGH:{
    	 ROS_INFO_STREAM_THROTTLE(1, "PASS_THROUGH...");
    ...
    joystick_output = joystick_input;
 	//detecting wall
    if  (detect(pose_local.pose.position.x,pose_local.pose.position.y,pose_local.pose.position.z) < delta){
		if(detect_level(pose_local.pose.position.x,pose_local.pose.position.y) < delta){
			touch_form=1;
		} 
		else{touch_form=0;}
		touch_point.x=pose_local.pose.position.x;
		touch_point.y=pose_local.pose.position.y;
		touch_point.z=pose_local.pose.position.z;
		touch_point.throttle=joystick_output.axes[JOY_CHANNEL_THROTTLE];
	...
	//back to touch point 
	double g_x,g_y,g_z,roll_sp_g,pitch_sp_g,throttle_sp_g,yaw_sp_g;
	tfScalar yaw_q, pitch_q, roll_q;
	tf::Matrix3x3(transform.getRotation()).getRPY(roll_q, pitch_q, yaw_q);
	

	g_x=((touch_point.x-pose_local.pose.position.x)*cos(yaw_q)-(touch_point.y-pose_local.pose.position.y)*sin(yaw_q));
	g_y=((touch_point.y-pose_local.pose.position.y)*cos(yaw_q)+(touch_point.x-pose_local.pose.position.x)*sin(yaw_q));
	g_z=touch_point.z-pose_local.pose.position.z;//transformation 
 
	controller_pose_x.update(saturate(g_x,  -2, 2), 0, ros_dt);
	controller_pose_y.update(saturate(-g_y, -2, 2), 0, ros_dt);
	controller_pose_z.update(saturate(g_z, -2, 2), 0, ros_dt);
	controller_pose_yaw.update(saturate(-yaw_q, -M_PI, M_PI), 0, ros_dt);
 
	roll_sp_g     = saturate(controller_pose_y.output, -0.6, 0.6) + goal_euler.x/EULER2JOY;
	pitch_sp_g    = saturate(controller_pose_x.output, -0.6, 0.6) - goal_euler.y/EULER2JOY;
	throttle_sp_g = saturate(controller_pose_z.output, -0.6, 0.6) + TAKEOFF_THROTTLE;
	yaw_sp_g      = saturate(controller_pose_yaw.output, -0.6, 0.6) + goal_euler.z/YAWRATE2JOY;
 
	joystick_output.axes[JOY_CHANNEL_PITCH]    = saturate(pitch_sp_g,   -1, 1);
	joystick_output.axes[JOY_CHANNEL_ROLL]     = saturate(roll_sp_g,    -1, 1);
	joystick_output.axes[JOY_CHANNEL_THROTTLE] = saturate(throttle_sp_g, 0, 1);
	joystick_output.axes[JOY_CHANNEL_YAW]      = saturate(yaw_sp_g,     -1, 1);
	if(detect_point(touch_point.x,touch_point.y,touch_point.z,
		pose_local.pose.position.x,pose_local.pose.position.y,pose_local.pose.position.z) < 4*delta)
	{
	   //direction jugdement
		if(touch_form==0){
			if(touch_point.z>(wz1+wz2)/2){
				if((hover_throttle*0.7 < joystick_input.axes[JOY_CHANNEL_THROTTLE])&&(joystick_input.axes[JOY_CHANNEL_THROTTLE] < hover_throttle)){
					joystick_output = joystick_input;
				  if(detect(pose_local.pose.position.x,pose_local.pose.position.y,pose_local.pose.position.z) >= delta){
				 	flight_mode = MANUAL;
					controller_pose_x.controllerFirstRun = true;
				...
				if((hover_throttle < joystick_input.axes[JOY_CHANNEL_THROTTLE])&&(joystick_input.axes[JOY_CHANNEL_THROTTLE] < 1.3*hover_throttle)){
					joystick_output = joystick_input;
				  if(detect(pose_local.pose.position.x,pose_local.pose.position.y,pose_local.pose.position.z) >= delta){
				  flight_mode = MANUAL;
					controller_pose_x.controllerFirstRun = true;
			...
			if((fabs(joystick_input.axes[JOY_CHANNEL_PITCH]) >=JOY_NOB_THRESHOLD)||( fabs(joystick_input.axes[JOY_CHANNEL_ROLL]) >=JOY_NOB_THRESHOLD)){ 
				double k_x,k_y,k_z;
	            tfScalar yaw_k, pitch_k, roll_k;
				tf::Quaternion q_k(pose_local.pose.orientation.x,
				pose_local.pose.orientation.y,
				pose_local.pose.orientation.z,
				pose_local.pose.orientation.w);
				tf::Matrix3x3 mat(q_k);
				mat.getEulerYPR(yaw_k, pitch_k, roll_k);
	
				k_x=joystick_input.axes[JOY_CHANNEL_PITCH]*cos(yaw_k)+joystick_input.axes[JOY_CHANNEL_ROLL]*sin(yaw_k);
				k_y=joystick_input.axes[JOY_CHANNEL_ROLL]*cos(yaw_k)-joystick_input.axes[JOY_CHANNEL_PITCH]*sin(yaw_k);
	
				if(angle(k_x,k_y,-pose_local.pose.position.x, pose_local.pose.position.y) >= conangle)
				{
					joystick_output = joystick_input;
					if(detect(pose_local.pose.position.x,pose_local.pose.position.y,pose_local.pose.position.z) >= delta){  
					flight_mode = MANUAL ;
	...
	idletime = totaltime;
      ...
      double x_d,y_d,z_d,roll_sp_d,pitch_sp_d,throttle_sp_d,yaw_sp_d;
	                tfScalar yaw_d, pitch_d, roll_d;
			 tf::Matrix3x3(transform.getRotation()).getRPY(roll_d, pitch_d, yaw_d);
	
			x_d=goal_local.pose.position.x*cos(yaw_d)-goal_local.pose.position.y*sin(yaw_d);
			y_d=goal_local.pose.position.y*cos(yaw_d)+goal_local.pose.position.x*sin(yaw_d);
			z_d=pose_local.pose.position.z-goal_local.pose.position.z;
		controller_pose_x.update(saturate(x_d,  -2, 2), 0, ros_dt);
		controller_pose_y.update(saturate(-y_d, -2, 2), 0, ros_dt);
		controller_pose_z.update(saturate(z_d, -2, 2), 0, ros_dt);
		controller_pose_yaw.update(saturate(-yaw_d, -M_PI, M_PI), 0, ros_dt);
 
		roll_sp_d     = saturate(controller_pose_y.output, -0.6, 0.6) + goal_euler.x/EULER2JOY;
		pitch_sp_d    = saturate(controller_pose_x.output, -0.6, 0.6) - goal_euler.y/EULER2JOY;
		throttle_sp_d = saturate(controller_pose_z.output, -0.6, 0.6) + TAKEOFF_THROTTLE;
		yaw_sp_d      = saturate(controller_pose_yaw.output, -0.6, 0.6) + goal_euler.z/YAWRATE2JOY;
 
		joystick_output.axes[JOY_CHANNEL_PITCH]    = saturate(pitch_sp_d,   -1, 1);
		joystick_output.axes[JOY_CHANNEL_ROLL]     = saturate(roll_sp_d,    -1, 1);
		joystick_output.axes[JOY_CHANNEL_THROTTLE] = saturate(throttle_sp_d, 0, 1);
		joystick_output.axes[JOY_CHANNEL_YAW]      = saturate(yaw_sp_d,     -1, 1);
	...
	if((( fabs(joystick_input.axes[JOY_CHANNEL_PITCH]) >=JOY_NOB_THRESHOLD)||( fabs(joystick_input.axes[JOY_CHANNEL_ROLL]) >=JOY_NOB_THRESHOLD))&&(fabs(joystick_input.axes[JOY_CHANNEL_THROTTLE]- joystick_output.axes[JOY_CHANNEL_THROTTLE])<JOY_NOB_THRESHOLD)){
			hover_throttle= joystick_output.axes[JOY_CHANNEL_THROTTLE]; 
			flight_mode = MANUAL;		}
		break; 
    } 
    case AUTO_IDLE:{
